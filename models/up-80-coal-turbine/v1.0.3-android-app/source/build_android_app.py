...
def write_docs(catalog: dict) -> None:
    ...
    write_text(ROOT / "RELEASE_NOTES.md", """
        ...
        Native Android catalog app source for the UP #80 model kit.

        ## Added
        - Android app project with Java source.
        - Local lightweight preview catalog generated from v1.0.1 STL files.
        - Three-shell default preview.
        - Full catalog preview.
        - GitHub links for each part, each batch, chassis docs, and the full v1.0.1 manufacturing zip.

        ## Notes
        - This is an Android app source release; no full STL files are bundled in the APK assets.
        - Printable files remain in v1.0.1.
...
def write_workflow() -> None:
    workflow = REPO_ROOT / ".github" / "workflows" / "up80-android-v103.yml"
    write_text(workflow, """
        name: Build UP80 Android v1.0.3
          ...
          build:
            ...
            steps:
              ...
              - name: Build debug APK
                run: gradle -p models/up-80-coal-turbine/v1.0.3-android-app/app-project :app:assembleDebug
              - name: Copy APK
                run: |
                  mkdir -p dist
                  cp models/up-80-coal-turbine/v1.0.3-android-app/app-project/app/build/outputs/apk/debug/app-debug.apk dist/up80-catalog-v1.0.3-debug.apk
              - name: Upload APK to release
                uses: softprops/action-gh-release@v2
                if: startsWith(github.ref, 'refs/tags/')
                with:
                  tag_name: v1.0.3
                  name: v1.0.3 Android App
                  files: dist/up80-catalog-v1.0.3-debug.apk
                  make_latest: true
    """)
...
def write_release_zip() -> Path:
    ...
    zip_path = release / "up80-android-app-v1.0.3-source.zip"
    if zip_path.exists():
        zip_path = release / f"up80-android-app-v1.0.3-source-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}.zip"

...
def write_docs(params: dict, manifest: dict) -> None:
    ...
    # HO Union Pacific #80 Coal Turbine v1.0.1
    ...
    v1.0.1 changes the file organization to match a practical printer handoff: one shell STL per unit, and many separate chassis parts per unit.
    ...
    Improved printable kit package for the HO Union Pacific #80 coal turbine.

    ## Added
    - One shell STL per unit.
    - Split chassis parts for all three units.
    - Chassis part index and updated build guide.
    - 3MF print-set previews, faceted STEP references, DXF, SVG, BOM, paint/decal plan, research notes, manifest, and validation report.
...
def write_release_zip() -> Path:
    ...
    zip_path = release_dir / "up-80-coal-turbine-ho-v1.0.1.zip"
    if zip_path.exists():
        zip_path = release_dir / f"up-80-coal-turbine-ho-v1.0.1-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}.zip"

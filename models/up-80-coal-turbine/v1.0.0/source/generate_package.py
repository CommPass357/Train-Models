...
def make_body_shell(unit_key: str, unit: dict, variant: str) -> Mesh:
    ...
    if unit_key == "a_control":
        ...
        add_box(m, (x_front + 16.0, 9.5, 29.0), (1.2, 5.5, 3.0))
        add_cylinder(m, (x_front + 3.2, 0, 22.8), 1.35, 1.2, 18, "x", True)
        add_cylinder(m, (x_front + 5.5, 0, 27.0), 1.1, 1.0, 18, "x", True)
        for side in (-1, 1):
            add_side_grilles(m, L, W, 20.0, x_body0 + 30.0, L / 2 - 24.0, 14 if detail else 8, side)
            add_box(m, (x_body0 + 13.0, side * (W / 2 + 0.25), 26.0), (8.0, 0.55, 5.0))
            add_box(m, (L / 2 - 18.0, side * (W / 2 + 0.25), 17.0), (10.0, 0.55, 12.0))
        ...
        for side in (-1, 1):
            add_side_grilles(m, L, W, 23.0, -L / 2 + 26.0, L / 2 - 26.0, 32 if detail else 16, side, tall=True)
            add_box(m, (0, side * (W / 2 + 0.7), 8.5), (L - 28.0, 1.0, 2.2))
            for x in (-L / 2 + 34, -L / 4, 0, L / 4, L / 2 - 34):
                add_box(m, (x, side * (W / 2 + 0.35), 17.0), (8.5, 0.6, 13.0))
        ...
        for side in (-1, 1):
            for x in (-L / 2 + 20, -L / 2 + 58, -L / 2 + 96, L / 2 - 32):
                add_box(m, (x, side * (W / 2 + 0.35), 17.0), (12.0, 0.55, 10.5))
...
def write_step_from_mesh(mesh: Mesh, path: Path) -> None:
    ...
    lines = [
        "ISO-10303-21;",
        "HEADER;",
        "FILE_DESCRIPTION(('Faceted chassis interchange generated from the v1.0.0 parametric source'),'2;1');",
...
def write_docs(params: dict, manifest: dict) -> None:
    ...
    # HO Union Pacific #80 Coal Turbine v1.0.0

    This package is a complete digital manufacturing set for a three-unit HO scale Union Pacific #80 coal turbine: A/control unit, center turbine unit, and Challenger-style coal tender. Each unit has its own shell and chassis file set.
    ...
    - High-detail resin STL files in `exports/stl/resin/`.
    - FDM/plastic STL and 3MF files in `exports/stl/fdm/` and `exports/3mf/`.
    - Faceted STEP interchange files and metal plate DXF in `exports/step/` and `exports/dxf/`.
    - Paint masks, decal placement guides, glazing guides, BOM, research notes, and build notes in `docs/`.

    ## Important Limits
    These files are a detailed digital model package generated from public dimensional references and photo/model-informed proportions. Final fit depends on the donor trucks, motors, couplers, printer, resin/plastic shrink, and the builder's chosen wiring. The chassis are functional HO designs, not a historical reproduction of the prototype underframe mechanics.
    """)
    doc(docs / "BOM.md", """
    # Bill of Materials
    ...
    ## Printing
    - Use `exports/stl/resin/` for high-detail shell work.
    - Use `exports/stl/fdm/` or `exports/3mf/` for stronger plastic prototypes.
    - Print small details from `up80_detail_sprue_resin.stl` separately for cleaner painting and easier replacement.
    - Print `up80_test_coupons.stl` before committing to full shells.

    ## Chassis
    - The A/control and center turbine chassis are powered-ready adapter plates.
    - The tender chassis is rolling/unpowered by default, with lighting, pickup, speaker, and keep-alive routing provisions.
    ...
    - Exact side grille count, small door placement, roof equipment spacing, and underframe equipment are photo/model-informed approximations.
    ...
    Initial complete digital manufacturing package for the HO Union Pacific #80 coal turbine.

    ## Added
    - Three unit shells: A/control, center turbine, and coal tender.
    - Three independent chassis: powered-ready A/control, powered-ready center turbine, rolling tender.
    - Resin STL, FDM STL, 3MF, faceted STEP, DXF, and SVG manufacturing outputs.
    - BOM, build guide, paint/decal plan, research notes, manifest, and validation report.
...
def write_release_zip() -> Path:
    ...
    zip_path = release_dir / "up-80-coal-turbine-ho-v1.0.0.zip"
    if zip_path.exists():
        zip_path = release_dir / f"up-80-coal-turbine-ho-v1.0.0-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}.zip"
...
def generate() -> None:
    ...
    for rel in ["exports/stl/resin", "exports/stl/fdm", "exports/3mf", "exports/step", "exports/dxf", "exports/svg", "docs", "release"]:
        (ROOT / rel).mkdir(parents=True, exist_ok=True)
    meshes: dict[str, Mesh] = {}
    for variant in ("resin", "fdm"):
        for key, unit in params["units"].items():
            shell = make_body_shell(key, unit, variant)
            chassis = make_chassis(key, unit, variant)
            meshes[shell.name] = shell
            meshes[chassis.name] = chassis
            shell.write_stl(ROOT / "exports" / "stl" / variant / f"{shell.name}.stl")
            chassis.write_stl(ROOT / "exports" / "stl" / variant / f"{chassis.name}.stl")
            if variant == "fdm":
                combined = Mesh(f"{key}_fdm_print_set")
                combined.extend(shell)
                shifted = Mesh(chassis.name)
                shifted.triangles = [tuple((v[0], v[1] + unit["shell_width_mm"] + 12.0, v[2]) for v in tri) for tri in chassis.triangles]
                combined.extend(shifted)
                write_3mf(combined, ROOT / "exports" / "3mf" / f"{key}_fdm_print_set.3mf")
            if variant == "resin":
                write_step_from_mesh(chassis, ROOT / "exports" / "step" / f"{key}_chassis_faceted.step")
        sprue = make_detail_sprue(params, variant)
        meshes[sprue.name] = sprue
        sprue.write_stl(ROOT / "exports" / "stl" / variant / f"{sprue.name}.stl")
    coupons = make_test_coupons()
    meshes[coupons.name] = coupons
    coupons.write_stl(ROOT / "exports" / "stl" / "resin" / "up80_test_coupons.stl")

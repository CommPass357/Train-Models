    ...
    write_text(package_dir / "MainActivity.java", r"""
        ...
        public final class MainActivity extends Activity {
            ...
            @Override protected void onCreate(Bundle state) {
                ...
                title.setText("UP #80 Catalog v1.0.4");
                ...
                row3.addView(button("v1.0.4 release", v -> open(catalog.manufacturingReleaseUrl)));
...
def write_docs(catalog: dict) -> None:
    ...
    write_text(ROOT / "README.md", f"""
        # UP #80 Android Catalog v1.0.4

        This release updates the lean native Android app and points it at the v1.0.4 manufacturing package.
        ...
        ## Build Status
        This machine did not have Android SDK/Gradle installed, so no local APK was produced. The repo includes an Android Studio/Gradle project and a GitHub Actions workflow intended to build a debug APK from tag v1.0.4.
    """)
    ...
    write_text(ROOT / "RELEASE_NOTES.md", """
        # v1.0.4 Android App + Shell Detail

        Native Android catalog app source for the UP #80 model kit, refreshed against the v1.0.4 shell-detail manufacturing files.

        ## Added
        - Android immersive mode so phone navigation buttons hide and return by edge swipe.
...
def write_workflow() -> None:
    workflow = REPO_ROOT / ".github" / "workflows" / "up80-android-v104.yml"
    write_text(workflow, """
        name: Build UP80 Android v1.0.4
          ...
          build:
            ...
            steps:
              ...
              - name: Build debug APK
                run: gradle -p models/up-80-coal-turbine/v1.0.4-android-app/app-project :app:assembleDebug
              - name: Copy APK
                run: |
                  mkdir -p dist
                  cp models/up-80-coal-turbine/v1.0.4-android-app/app-project/app/build/outputs/apk/debug/app-debug.apk dist/up80-catalog-v1.0.4-debug.apk
              - name: Upload APK to release
                uses: softprops/action-gh-release@v2
                if: startsWith(github.ref, 'refs/tags/')
                with:
                  tag_name: v1.0.4
                  name: v1.0.4 Android App + Shell Detail
                  files: dist/up80-catalog-v1.0.4-debug.apk
                  make_latest: true
    """)
...
def write_release_zip() -> Path:
    ...
    zip_path = release / "up80-android-app-v1.0.4-source.zip"
    if zip_path.exists():
        zip_path = release / f"up80-android-app-v1.0.4-source-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}.zip"

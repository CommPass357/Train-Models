...
def write_java(package_dir: Path) -> None:
    ...
    write_text(package_dir / "MainActivity.java", r"""
        ...
        public final class MainActivity extends Activity {
            private static final String APP_RELEASE_URL = "https://github.com/CommPass357/Train-Models/releases/tag/v1.0.5";
            ...
            @Override protected void onCreate(Bundle state) {
                ...
                title.setText("UP #80 Catalog v1.0.5");
            ...
            private void showUpdateDialog() {
                new AlertDialog.Builder(this)
                    ...
                    .setMessage("App v1.0.5 previews the v1.0.4 model package. Open GitHub to get the newest APK, source, or printable model files.")
...
def write_docs(catalog: dict) -> None:
    ...
    write_text(ROOT / "RELEASE_NOTES.md", """
        # v1.0.5 Android Update Notice

        Native Android catalog app source for the UP #80 model kit, refreshed with a small GitHub update notice.

        ## Added
        - Small `!` button in the top title row.
        - Native update popup with links to the latest GitHub release and the current app release.
...
def write_workflow() -> None:
    workflow = REPO_ROOT / ".github" / "workflows" / "up80-android-v105.yml"
    write_text(workflow, """
        name: Build UP80 Android v1.0.5
          ...
          build:
            ...
            steps:
              ...
              - name: Build debug APK
                run: gradle -p models/up-80-coal-turbine/v1.0.5-android-app/app-project :app:assembleDebug
              - name: Copy APK
                run: |
                  mkdir -p dist
                  cp models/up-80-coal-turbine/v1.0.5-android-app/app-project/app/build/outputs/apk/debug/app-debug.apk dist/up80-catalog-v1.0.5-debug.apk
              - name: Upload APK to release
                uses: softprops/action-gh-release@v2
                if: startsWith(github.ref, 'refs/tags/')
                with:
                  tag_name: v1.0.5
                  name: v1.0.5 Android Update Notice
                  files: dist/up80-catalog-v1.0.5-debug.apk
                  make_latest: true
    """)
...
def write_release_zip() -> Path:
    ...
    zip_path = release / "up80-android-app-v1.0.5-source.zip"
    if zip_path.exists():
        zip_path = release / f"up80-android-app-v1.0.5-source-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}.zip"
